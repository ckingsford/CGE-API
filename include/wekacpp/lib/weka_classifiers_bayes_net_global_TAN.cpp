...
#include <weka/classifiers/bayes/net/global/TAN.h>
#include <stdexcept>
...
TAN::buildStructure (BayesNet * bayesNet, Instances * instances)
{
	Debug.dprintf("%s: starting: %d attributes %d classes\n", 
		"TAN::buildStructure [global]", instances->numAttributes(),
	...
	m_nMaxNrOfParents = 2;
	GlobalScoreSearchAlgorithm::buildStructure(bayesNet, instances);
		...
		if (iLinkNode1 != nClassNode) {
			for (int iLinkNode2 = 0; iLinkNode2 < nNrOfAtts; iLinkNode2++) {
				if ((iLinkNode1 != iLinkNode2) && (iLinkNode2 != nClassNode)) {
					double fScore = 
						calcScoreWithExtraParent(iLinkNode1, iLinkNode2);
